...
public class Grafo<T extends Comparable<T>> {
	...
	public void recorrerEnProfundidad() {
		if (vertices.get(0) == null) {
			...
			while (stack.size() > 0) {
				...
				if (verticeAExplorar.esVisitado() == false) {
				...
				for (Arista<T> arista : aristasAVisitar) {
					if (!arista.getV1().esVisitado()) {
						stack.add(arista.getV1());
					}
					if (!arista.getV2().esVisitado()) {
	...
	public void recorrerEnAnchura() {
		if (vertices.get(0) == null) {
			...
			while (queue.size() > 0) {
				Vertice<T> verticeAExplorar = queue.poll();
				if (verticeAExplorar.esVisitado() == false) {
					System.out.println(verticeAExplorar.getElemento());
				}
				...
				for (Arista<T> arista : aristasAVisitar) {
					if (!arista.getV1().esVisitado()) {
						queue.add(arista.getV1());
					}
					if (!arista.getV2().esVisitado()) {
						queue.add(arista.getV2());
					}
				}
	...
	public void buscarVertice(T aEncontrar) {
		if (vertices.get(0) == null) {
			System.out.println("El grafo está vacío");
		} else {
			queue.add(vertices.get(0));
			while (queue.size() > 0) {
				Vertice<T> verticeAExplorar = queue.poll();
				if (verticeAExplorar.esVisitado() == false) {
					if(verticeAExplorar.getElemento().compareTo(aEncontrar)==0) {
						System.out.println("Elemento: " + aEncontrar.toString() + " encontrado.");
						return;
					}
